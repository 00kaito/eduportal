...
', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('What is polymorphism', 'Polymorphism is the ability of programming languages to present the same interface for differing underlying data types. A
polymorphic type is a type whose operations can also be applied to values of some other type.

', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('What is inheritance', 'Inheritance provides an object with the ability to acquire the fields and methods of another class, called base class. Inheritance
provides re-usability of code and can be used to add additional features to an existing class, without modifying it.

', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('What is abstraction', 'Abstraction is the process of separating ideas from specific instances and thus, develop classes in terms of their own functionality,
...
', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('Differences between Abstraction and Encapsulation', 'Abstraction and encapsulation are complementary concepts. On the one hand, abstraction focuses on the behavior of an object.
...
', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('What is JVM ? Why is Java called the Platform Independent Programming Language?', 'A Java virtual machine (JVM) is a process virtual machine that can execute Java bytecode. Each Java source file is compiled
...
', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('Can you access non static variable in static context ?', 'A static variable in Java belongs to its class and its value remains the same for all its instances. A static variable is initialized
...
', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('What are the Data Types supported by Java ? What is Autoboxing and Unboxing ?', 'The eight primitive data types supported by the Java programming language are:
• byte
• short
• int
• long
...
', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('What is Function Overriding and Overloading in Java ?', 'Method overloading in Java occurs when two or more methods in the same class have the exact same name, but different
...
', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('What is a Constructor, Constructor Overloading in Java and Copy-Constructor', 'A constructor gets invoked when a new object is created. Every class has a constructor. In case the programmer does not provide
...
', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('Does Java support multiple inheritance ?', 'No, Java does not support multiple inheritance. Each class is able to extend only on one class, but is able to implement more than
one interfaces.

', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('What is the difference between an Interface and an Abstract class ?', 'Java provides and supports the creation both of abstract classes and interfaces. Both implementations share some common
...
', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('What are pass by reference and pass by value ?', 'When an object is passed by value, this means that a copy of the object is passed. Thus, even if changes are made to that object,
...
', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('What is the difference between processes and threads ?', 'A process is an execution of a program, while a Thread is a single execution sequence within a process. A process can contain
multiple threads. A Thread is sometimes called a lightweight process.

', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('Explain different ways of creating a thread. Which one would you prefer and why ?', 'There are three ways that can be used in order for a Thread to be created:
...
', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('Explain the available thread states in a high-level.', 'During its execution, a thread can reside in one of the following states:
...
', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('What is the difference between a synchronized method and a synchronized block ?', 'In Java programming, each object has a lock. A thread can acquire the lock for an object by using the synchronized keyword.
The synchronized keyword can be applied in a method level (coarse grained lock) or block level of code (fine grained lock).

', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('How does thread synchronization occurs inside a monitor ? What levels of synchronization can you apply ?', 'The JVM uses locks in conjunction with monitors. A monitor is basically a guardian that watches over a sequence of synchronized
...
', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('What’s a deadlock ?', 'A condition that occurs when two processes are waiting for each other to complete, before proceeding. The result is that both
processes wait endlessly.

', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('How do you ensure that N threads can access N resources without deadlock ?', 'A very simple way to avoid deadlock while using N threads is to impose an ordering on the locks and force each thread to follow
that ordering. Thus, if all threads lock and unlock the mutexes in the same order, no deadlocks can arise.


', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('What are the basic interfaces of Java Collections Framework ?', 'Java Collections Framework provides a well designed set of interfaces and classes that support operations on a collections of
...
', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('Why Collection doesn’t extend Cloneable and Serializable interfaces ?', 'The Collection interface specifies groups of objects known as elements. Each concrete implementation of a Collection can choose
...
', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('What is an Iterator ?', 'The Iterator interface provides a number of methods that are able to iterate over any Collection. Each Java Collection contains
...
', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('What differences exist between Iterator and ListIterator ?', 'The differences of these elements are listed below:
...
', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('What is difference between fail-fast and fail-safe ?', 'The Iterator’s fail-safe property works with the clone of the underlying collection and thus, it is not affected by any modification
...
', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('How HashMap works in Java ?', 'A HashMap in Java stores key-value pairs. The HashMap requires a hash function and uses hashCode and equals methods,
...
', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('What is the importance of hashCode() and equals() methods ?', 'In Java, a HashMap uses the hashCode and equals methods to determine the index of the key-value pair and to detect duplicates.
...
', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('What differences exist between HashMap and Hashtable ?', 'Both the HashMap and Hashtable classes implement the Map interface and thus, have very similar characteristics. However, they
...
', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('What is difference between Array and ArrayList ? When will you use Array over ArrayList ?', 'The Array and ArrayList classes differ on the following features:
...
', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('What is difference between ArrayList and LinkedList ?', 'Both the ArrayList and LinkedList classes implement the List interface, but they differ on the following features:
...
', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('What is Comparable and Comparator interface ? List their differences.', 'Java provides the Comparable interface, which contains only one method, called compareTo. This method compares two objects,
...
', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('What is Java Priority Queue ?', 'The PriorityQueue is an unbounded queue, based on a priority heap and its elements are ordered in their natural order. At the time
...
', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('What do you know about the big-O notation and can you give some examples with respect to different data structures ?', 'The Big-O notation simply describes how well an algorithm scales or performs in the worst case scenario as the number of ele-
...
', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('What is the tradeoff between using an unordered array versus an ordered array ?', 'The major advantage of an ordered array is that the search times have time complexity of O(log n), compared to that of an
...
', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('What are some of the best practices relating to the Java Collection framework ?', '• Choosing the right type of the collection to use, based on the application’s needs, is very crucial for its performance. For
...
', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('What’s the difference between Enumeration and Iterator interfaces ?', 'Enumeration is twice as fast as compared to an Iterator and uses very less memory. However, the Iterator is much safer compared
...
', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('What is the difference between HashSet and TreeSet ?', 'The HashSet is Implemented using a hash table and thus, its elements are not ordered. The add, remove, and contains methods of
...
', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('What is the purpose of garbage collection in Java, and when is it used ?', 'The purpose of garbage collection is to identify and discard those objects that are no longer needed by the application, in order
for the resources to be reclaimed and reused.

', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('What does System.gc() and Runtime.gc() methods do ?', 'These methods can be used as a hint to the JVM, in order to start a garbage collection. However, this it is up to the Java Virtual
Machine (JVM) to start the garbage collection immediately or later in time.

', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('When is the finalize() called ? What is the purpose of finalization ?', 'The finalize method is called by the garbage collector, just before releasing the object’s memory. It is normally advised to release
resources held by the object inside the finalize method.

', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('If an object reference is set to null, will the Garbage Collector immediately free the memory held by that object ?', 'No, the object will be available for garbage collection in the next cycle of the garbage collector.

', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('What is structure of Java Heap ? What is Perm Gen space in Heap ?', 'The JVM has a heap that is the runtime data area from which memory for all class instances and arrays is allocated. It is created
...
', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('What is the difference between Serial and Throughput Garbage collector ?', 'The throughput garbage collector uses a parallel version of the young generation collector and is meant to be used with applica-
...
', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('When does an Object becomes eligible for Garbage collection in Java ?', 'A Java object is subject to garbage collection when it becomes unreachable to the program in which it is currently used.

', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('Does Garbage collection occur in permanent generation space in JVM ?', 'Garbage Collection does occur in PermGen space and if PermGen space is full or cross a threshold, it can trigger a full garbage
...
', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('What are the two types of Exceptions in Java ? Which are the differences between them ?', 'Java has two types of exceptions: checked exceptions and unchecked exceptions. Unchecked exceptions do not need to be
...
', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('What is the importance of finally block in exception handling ?', 'A finally block will always be executed, whether or not an exception is actually thrown. Even in the case where the catch
...
', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('What will happen to the Exception object after exception handling ?', 'The Exception object will be garbage collected in the next garbage collection.

', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('How does finally block differ from finalize() method ?', 'A finally block will be executed whether or not an exception is thrown and is used to release those resources held by the
...
', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('What is an Applet ?', 'A java applet is program that can be included in a HTML page and be executed in a java enabled client browser. Applets are used
for creating dynamic and interactive web applications.

', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('Explain the life cycle of an Applet.', 'An applet may undergo the following states:
...
', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('What happens when an applet is loaded ?', 'First of all, an instance of the applet’s controlling class is created. Then, the applet initializes itself and finally, it starts running.

', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('What is the difference between an Applet and a Java Application ?', 'Applets are executed within a java enabled browser, but a Java application is a standalone Java program that can be executed
...
', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('What are the restrictions imposed on Java applets ?', 'Mostly due to security reasons, the following restrictions are imposed on Java applets:
...
', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('What are untrusted applets ?', 'Untrusted applets are those Java applets that cannot access or execute local system files. By default, all downloaded applets are
considered as untrusted.

', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('What is the difference between applets loaded over the internet and applets loaded via the file system ?', 'Regarding the case where an applet is loaded over the internet, the applet is loaded by the applet classloader and is subject to the
...
', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('What is the applet class loader, and what does it provide ?', 'When an applet is loaded over the internet, the applet is loaded by the applet classloader. The class loader enforces the Java name
...
', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('What is the applet security manager, and what does it provide ?', 'The applet security manager is a mechanism to impose restrictions on Java applets. A browser may only have one security
...
', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('What is a layout manager ?', 'A layout manager is the used to organize the components in a container.

', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('What is the difference between a Scrollbar and a JScrollPane ?', 'A Scrollbar is a Component, but not a Container. A ScrollPane is a Container. A ScrollPane handles its own events and performs
its own scrolling.

', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('Which Swing methods are thread-safe ?', 'There are only three thread-safe methods: repaint, revalidate, and invalidate.

', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('Name three Component subclasses that support painting.', 'The Canvas, Frame, Panel, and Applet classes support painting.

', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('What is clipping ?', 'Clipping is defined as the process of confining paint operations to a limited area or shape.

', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('What is the difference between a MenuItem and a CheckboxMenuItem ?', 'The CheckboxMenuItem class extends the MenuItem class and supports a menu item that may be either checked or unchecked.

', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('How are the elements of a BorderLayout organized ?', 'The elements of a BorderLayout are organized at the borders (North, South, East, and West) and the center of a container.

', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('How are the elements of a GridBagLayout organized ?', 'The elements of a GridBagLayout are organized according to a grid. The elements are of different sizes and may occupy more
than one row or column of the grid. Thus, the rows and columns may have different sizes.

', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('What is the difference between a Window and a Frame ?', 'The Frame class extends the Window class and defines a main application window that can have a menu bar.

', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('What is the relationship between clipping and repainting ?', 'When a window is repainted by the AWT painting thread, it sets the clipping regions to the area of the window that requires
repainting.

', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('What is the relationship between an event-listener interface and an eventadapter class ?', 'An event-listener interface defines the methods that must be implemented by an event handler for a particular event. An event
adapter provides a default implementation of an event-listener interface.

', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('How can a GUI component handle its own events ?', 'A GUI component can handle its own events, by implementing the corresponding event-listener interface and adding itself as its
own event listener.

', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('What advantage do Java’s layout managers provide over traditional windowing systems ?', 'Java uses layout managers to lay out components in a consistent manner, across all windowing platforms. Since layout man-
...
', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('What is the design pattern that Java uses for all Swing components ?', 'The design pattern used by Java for all Swing components is the Model View Controller (MVC) pattern.

', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('What is JDBC ?', 'JDBC is an abstraction layer that allows users to choose between databases. JDBC enables developers to write database applica-
tions in Java, without having to concern themselves with the underlying details of a particular database.

', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('Explain the role of Driver in JDBC.', 'The JDBC Driver provides vendor-specific implementations of the abstract classes provided by the JDBC API. Each driver
...
', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('What is the purpose Class.forName method ?', 'This method is used to method is used to load the driver that will establish a connection to the database.

', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('What is the advantage of PreparedStatement over Statement ?', 'PreparedStatements are precompiled and thus, their performance is much better. Also, PreparedStatement objects can be reused
...
', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('What does Connection pooling mean ?', 'The interaction with a database can be costly, regarding the opening and closing of database connections. Especially, when the
...
', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('What are the layers of RMI Architecture ?', 'The RMI architecture consists of the following layers:
...
', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('What is the role of Remote Interface in RMI ?', 'The Remote interface serves to identify interfaces whose methods may be invoked from a non-local virtual machine. Any object
...
', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('What is the role of the java.rmi.Naming Class ?', 'The java.rmi.Naming class provides methods for storing and obtaining references to remote objects in the remote object registry.
Each method of the Naming class takes as one of its arguments a name that is a String in URL format.

', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('What is meant by binding in RMI ?', 'Binding is the process of associating or registering a name for a remote object, which can be used at a later time, in order to look
up that remote object. A remote object can be associated with a name using the bind or rebind methods of the Naming class.

', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('What is the difference between using bind() and rebind() methods of Naming', 'Class ?
...
', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('What are the steps involved to make work a RMI program ?', 'The following steps must be involved in order for a RMI program to work properly:
...
', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('What is the role of stub in RMI ?', 'A stub for a remote object acts as a client’s local representative or proxy for the remote object. The caller invokes a method on
...
', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('What is DGC ? And how does it work ?', 'DGC stands for Distributed Garbage Collection. Remote Method Invocation (RMI) uses DGC for automatic garbage collection.
...
', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('What is the purpose of using RMISecurityManager in RMI ?', 'RMISecurityManager provides a security manager that can be used by RMI applications, which use downloaded code. The class
loader of RMI will not download any classes from remote locations, if the security manager has not been set.

', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('Explain Marshalling and demarshalling.', 'When an application wants to pass its memory objects across a network to another host or persist it to storage, the in-memory
...
', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('Explain Serialization and Deserialization.', 'Java provides a mechanism, called object serialization where an object can be represented as a sequence of bytes and includes
...
', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('What is a Servlet ?', 'The servlet is a Java programming language class used to process client requests and generate dynamic web content. Servlets are
...
', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('Explain the architechure of a Servlet.', 'The core abstraction that must be implemented by all servlets is the javax.servlet.Servlet interface. Each servlet must implement
...
', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('Explain the life cycle of a Servlet.', 'On every client’s request, the Servlet Engine loads the servlets and invokes its init methods, in order for the servlet to be
...
', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('What is the difference between doGet() and doPost() ?', 'doGET: The GET method appends the name-value pairs on the request’s URL. Thus, there is a limit on the number of characters
...
', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('What is meant by a Web Application ?', 'A Web application is a dynamic extension of a Web or application server. There are two types of web applications: presentation-
...
', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('What is a Server Side Include (SSI) ?', 'Server Side Includes (SSI) is a simple interpreted server-side scripting language, used almost exclusively for the Web, and is
...
', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('What is Servlet Chaining ?', 'Servlet Chaining is the method where the output of one servlet is sent to a second servlet. The output of the second servlet can
be sent to a third servlet, and so on. The last servlet in the chain is responsible for sending the response to the client.

', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('How do you find out what client machine is making a request to your servlet ?', 'The ServletRequest class has functions for finding out the IP address or host name of the client machine. getRemoteAddr() gets
the IP address of the client machine and getRemoteHost() gets the host name of the client machine. See example here.

', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('What is the structure of the HTTP response ?', 'The HTTP response consists of three parts:
...
', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('What is a cookie ? What is the difference between session and cookie ?', 'A cookie is a bit of information that the Web server sends to the browser. The browser stores the cookies for each Web server
...
', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('Which protocol will be used by browser and servlet to communicate ?', 'The browser communicates with a servlet by using the HTTP protocol.

', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('What is HTTP Tunneling ?', 'HTTP Tunneling is a technique by which, communications performed using various network protocols are encapsulated using
...
', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('What’s the difference between sendRedirect and forward methods ?', 'The sendRedirect method creates a new request, while the forward method just forwards a request to a new target. The previous
...
', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('What is URL Encoding and URL Decoding ?', 'The URL encoding procedure is responsible for replacing all the spaces and every other extra special character of a URL, into
their corresponding Hex representation. In correspondence, URL decoding is the exact opposite procedure.


', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('What is a JSP Page ?', 'A Java Server Page (JSP) is a text document that contains two types of text: static data and JSP elements. Static data can be
...
', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('How are the JSP requests handled ?', 'On the arrival of a JSP request, the browser first requests a page with a .jsp extension. Then, the Web server reads the request and
...
', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('What are the advantages of JSP ?', 'The advantages of using the JSP technology are shown below:
...
', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('What are Directives ? What are the different types of Directives available in JSP ?', 'Directives are instructions that are processed by the JSP engine, when the page is compiled to a servlet. Directives are used to
...
', 1);
INSERT INTO question (QUESTION, ANSWER, COURSE_ID) VALUES ('What are Expressions ?', 'A JSP expression is used to insert the value of a scripting language expression, converted into a string, into the data stream
